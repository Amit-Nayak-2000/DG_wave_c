...
void DG_init(){
	...
	double xs = 1.5; //originally 1.5, 4.0 NASA
	...
	for(int p = 0; p <= grid::nmin; p++){
		...
		// Rectangle
		one_x[p] = rinf*nodal::gl_points[grid::nmin][p];
		one_y[p] = 0; 

		two_x[p] = rinf; 
		two_y[p] = rinf*(nodal::gl_points[grid::nmin][p] + 1)/2;

		three_x[p] = rinf*nodal::gl_points[grid::nmin][p];
		three_y[p] = rinf; 

		four_x[p] = -rinf; 
		four_y[p] = rinf*(nodal::gl_points[grid::nmin][p] + 1)/2;
	...
	for(int k = 0; k < local::local_elem_num; ++k){
		...
		for(int j = 0; j <= grid::nmin; ++j){
			...
			for(int i = 0; i <= grid::nmin; ++i){
				...
				temp -> solution[0][num_p] = exp( -log(2) * ( (std::pow((temp->holdmetrics.x_node[num_p] - xs), 2) + std::pow((temp->holdmetrics.y_node[num_p]), 2) ) / std::pow(w, 2) ) );
				//u = 0
